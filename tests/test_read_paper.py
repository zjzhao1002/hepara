...
class ReadPaperTest(unittest.IsolatedAsyncioTestCase):
    async def test_read_paper_generates_missing_markdown_for_stored_pdf(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch.object(tools, "PDF_PATH", tmpdir),
                patch.object(tools.pymupdf4llm, "to_markdown", return_value="paper markdown") as to_markdown,
            ):
    ...
    async def test_read_paper_returns_error_when_stored_pdf_conversion_fails(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch.object(tools, "PDF_PATH", tmpdir),
                patch.object(tools.pymupdf4llm, "to_markdown", side_effect=RuntimeError("boom")),
            ):
                content = await tools.read_paper("1234.5678")
    ...
    async def test_download_pdf_creates_destination_directory(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                ...
                patch.object(tools.pymupdf4llm, "to_markdown", return_value="paper markdown"),
            ):
                downloaded_path = await tools.download_pdf("1234.5678", dirpath=str(target_dir))
